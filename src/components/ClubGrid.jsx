...
export default function ClubGrid({
  ...
  const hasAdminEdit =
    new URLSearchParams(window.location.search).get("adminEdit") ===
    "gcnAdmin123";
            ...
            >
              <Flex justify="space-between" align="flex-start" mb="4px">
                {!imageError[club.id] ? (
                  <Image
                    ...
                    objectFit="contain"
                    onError={() =>
                      setImageError((prev) => ({ ...prev, [club.id]: true }))
                    }
                  />
                ) : (
                  <CategoryIcon
              ...
              {isSelected && (
                <Box mt="8px">
                  <LevelPicker
                    club={club}
                    level={bagEntry.level}
                    onChange={(lvl) => handleSetLevel(club, lvl)}
                    source="grid"
                  />
                </Box>
              )}
